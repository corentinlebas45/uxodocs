...
const listDirs = (dir: string) => (exists(dir) ? fs.readdirSync(dir, {withFileTypes: true}).filter(d => d.isDirectory()).map(d => d.name) : []);
const titleCase = (s: string) => (s ? s[0].toUpperCase() + s.slice(1) : s);

function buildProductCategory(product: string): SidebarItem | null {
  const productDir = path.join(DOCS_DIR, product);
  if (!hasIndex(productDir)) return null; // n’ajoute le produit que s’il a un index

  const items: SidebarItem[] = [{ type: 'doc', id: `${product}/index`, label: 'Index' }];
  ...
  return { type: 'category', label: titleCase(product), collapsed: false, items };
}

const sidebarItems: SidebarItem[] = [];

// Intro globale (facultatif)
if (exists(path.join(DOCS_DIR, 'intro.md')) || exists(path.join(DOCS_DIR, 'intro.mdx'))) {
  sidebarItems.push({ type: 'doc', id: 'intro', label: 'Introduction' });
}

// Un bloc par produit de la whitelist
