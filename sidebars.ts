...
function buildProductCategory(product: string): SidebarItem | null {
  ...
  // 2) Fichiers à la racine (frères de index) au même niveau que Index
  const rootDocs = fs
  ...
  // 3) Catégories racine (tous les sous-dossiers qui ne sont PAS des versions)
  const rootDirs = listDirs(productDir)
    .filter(name => !/^v\d+/i.test(name)) // exclut v*
    .sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
  for (const dir of rootDirs) {
    // on ajoute la catégorie seulement si elle contient au moins un doc
    if (exists(path.join(productDir, dir))) {
      items.push({
        type: 'category',
        label: dir,
        collapsed: true,
